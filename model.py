...
def train(model_path=None, epsilon_override=None):
    ...
    for episode in range(start_epoch, num_episodes):
        ...
        steps = 0
        step_penalty = 1
        max_steps = int(init_max_steps - ((init_max_steps - 100) / num_episodes) * episode)  # Gradually reduce the maximum steps per episode
        rewards_for_episode = []
...
if __name__ == "__main__":
    model_path = "policy_net_1460_v0_offgoalflag.pth"  # last model trained with non-sparse (turn-steady) rewards
