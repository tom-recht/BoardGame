...
def select_action(policy_logits, action_mask):
    masked_logits = policy_logits.masked_fill(~action_mask, -1e9)  # Mask invalid actions
    action_probs = F.softmax(masked_logits, dim=-1)
    action = torch.multinomial(action_probs, 1)
    return action.item()

def collect_trajectories(model, num_actions, max_steps=1000):
    ...
    while not done and step_count < max_steps:
        ...
        action = select_action(policy_logits, action_mask)  # Select action using mask
...
def action_index_to_move(action_index, board):
    ...
    return piece, destination, roll, board.current_player
...
for episode in range(num_episodes):
    states, actions, rewards, dones = collect_trajectories(model, num_actions)
    ...
    if episode % 10 == 0:  # Save checkpoint every 10 episodes
        save_model(model, f'GCN_checkpoint_{episode}.pt')
