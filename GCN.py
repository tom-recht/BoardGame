...
def save_checkpoint(model, optimizer, episode):
    filepath = f'GCN_checkpoint_batched_{episode}.pt'
    torch.save({
...
def collect_trajectories(model, num_actions, epsilon, max_steps=1000):
    ...
    logger.info(f'Total Reward: {sum(rewards)}, Number of Steps: {num_steps}')
...
if __name__ == '__main__':
    ...
    import glob
    checkpoint_files = glob.glob('GCN_checkpoint_batched_*.pt')
    if checkpoint_files:
