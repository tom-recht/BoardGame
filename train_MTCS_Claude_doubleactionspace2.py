# version 2 of double action space since version 1 went inexplicably wrong with reward assignment
# but this version still does the same
...
def save_checkpoint(model, optimizer, epoch, epsilon):
    ...
    while os.path.exists(f"{base_path}_v{version}.pth"):
        version += 1
    path = f"{base_path}_v{version}_DAS2.pth"
    torch.save({
        'epoch': epoch,
...
def train(model_path=None, epsilon_override=None):
    ...
    for episode in range(start_epoch, num_episodes):
        ...
        if episode % 100 == 0:
            epsilon = min(1.0, epsilon + 0.25)
        else:
            epsilon = max(min_epsilon, epsilon * epsilon_decay) 

        # Print episode information
        print("Double action space version 2")
...
if __name__ == "__main__":
    model_path = "policy_net_110_v0_DAS2.pth"  # last model trained with non-sparse (turn-steady) rewards
