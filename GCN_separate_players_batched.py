...
def save_checkpoint(model, optimizer, episode):
    filepath = f'GCN_checkpoint_separateplayers_batched_{episode}.pt'
    torch.save({
...
def collect_trajectories(model, num_actions, epsilon, max_steps=1000):
    states_white, actions_white, rewards_white, dones_white = [], [], [], []
    states_black, actions_black, rewards_black, dones_black = [], [], [], []
    ...
    while not done and step_count < max_steps:
        ...
        if current_player == 'white':
            ...
            actions_white.append(action)
            rewards_white.append(reward)
            dones_white.append(done)
        else:
            states_black.append(state)
            actions_black.append(action)
            rewards_black.append(reward)
            dones_black.append(done)

        step_count += 1

    num_steps = len(rewards_white) + len(rewards_black)
    logger.info(f'Total Reward: {sum(rewards_white) + sum(rewards_black)}, Number of Steps: {num_steps}')
    ...
    return (states_white, actions_white, rewards_white, dones_white), (states_black, actions_black, rewards_black, dones_black)
...
if __name__ == '__main__':
    ...
    # Set the number of episodes per batch
    ...
    import glob
    checkpoint_files = glob.glob('GCN_checkpoint_separateplayers_batched_*.pt')
    if checkpoint_files:
    ...
    num_episodes = 10000
    for batch_start in range(start_episode, num_episodes, episodes_per_batch):
        ...
        for episode in range(episodes_per_batch):
            logger.info(f"Episode {batch_start + episode}")
            (white_trajectories, black_trajectories) = collect_trajectories(model, num_actions, epsilon)
            states_white, actions_white, rewards_white, dones_white = white_trajectories
            states_black, actions_black, rewards_black, dones_black = black_trajectories

            batch_states.extend(states_white + states_black)
            batch_actions.extend(actions_white + actions_black)
            batch_rewards.extend(rewards_white + rewards_black)
            batch_dones.extend(dones_white + dones_black)

            # Log the total rewards and steps for each episode
            total_rewards_per_episode.append(sum(rewards_white) + sum(rewards_black))
            steps_per_episode.append(len(rewards_white) + len(rewards_black))
        # Compute values, advantages, and returns for the batch

        logger.info("Computing values, advantages, and returns")
        values = [model(state.x, state.edge_index, torch.cat([state.die_features, state.current_turn, state.stage_vector]).unsqueeze(0))[1].item() for state in batch_states]
        advantages, returns = compute_advantages(batch_rewards, values, batch_dones)

        # Update the model using the batch of trajectories
        logger.info("Updating the model")
        ppo_update(batch_states, batch_actions, batch_rewards, batch_dones, advantages, returns, model, optimizer)

        # Save checkpoint after every 5 batches
        if batch_start % 5 == 0:
            save_checkpoint(model, optimizer, batch_start)
            logger.info(f"Saved checkpoint at episode {batch_start}")

        # Decay epsilon
        epsilon = max(min_epsilon, epsilon * decay_rate)
        logger.info(f"Batch starting at episode {batch_start}: Epsilon: {epsilon}")
