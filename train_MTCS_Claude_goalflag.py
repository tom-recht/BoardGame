...
# try without increasing penalty for game length / decreasing max steps 
# lower learning rate from .0005 to .0002
...
def save_checkpoint(model, optimizer, epoch, epsilon):
    ...
    while os.path.exists(f"{base_path}_v{version}.pth"):
        version += 1
    path = f"{base_path}_v{version}_offgoalflag.pth"
    torch.save({
        'epoch': epoch,
...
def train(model_path=None, epsilon_override=None):
    ...
    for episode in range(start_epoch, num_episodes):
        ...
        if episode % 100 == 0:
            epsilon = min(1.0, epsilon + 0.5)
        else:
            epsilon = max(min_epsilon, epsilon * epsilon_decay) 

        # Print episode information
        print("Action space with offgoal flag")
        ...
        print(f"Total offgoals: {board.offgoals['white'] + board.offgoals['black']}\n")
...
if __name__ == "__main__":
    model_path = "policy_net_5210_v0_offgoalflag.pth"  # last model trained with non-sparse (turn-steady) rewards
    ...
    train(model_path, epsilon_override=1)
