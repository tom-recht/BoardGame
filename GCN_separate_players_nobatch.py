...
def save_checkpoint(model, optimizer, episode):
    filepath = f'GCN_checkpoint_separateplayers_Jul7_{episode}.pt'
    torch.save({
...
def ppo_update(trajectories, model, optimizer, clip_param=0.2, ppo_epochs=10, mini_batch_size=64):
    for trajectory in trajectories:
        states, actions, rewards, dones = trajectory

        # Compute advantages and returns
        with torch.no_grad():
            values = [model(state.x, state.edge_index, torch.cat([state.die_features, state.current_turn, state.stage_vector]).unsqueeze(0))[1] for state in states]
            advantages, returns = compute_advantages(rewards, values, dones)

        for _ in range(ppo_epochs):
            for state_batch_data, action_batch, reward_batch, done_batch, advantage_batch, return_batch, batch in mini_batch_generator(
                states, actions, rewards, dones, advantages, returns, mini_batch_size):

                global_features_batch = torch.cat([state_batch_data.die_features, state_batch_data.current_turn, state_batch_data.stage_vector], dim=1)
                
                policy_logits, values = model(state_batch_data.x, state_batch_data.edge_index, global_features_batch, batch=batch)
                ...
                surr2 = torch.clamp(ratios, 1.0 - clip_param, 1.0 + clip_param) * advantage_batch
                policy_loss = -torch.min(surr1, surr2).mean()

                # Calculate the value loss
                value_loss = F.mse_loss(values.squeeze(-1), return_batch)

                # Calculate the entropy (optional, but usually used to encourage exploration)
...
if __name__ == '__main__':
    ...
    import glob
    checkpoint_files = glob.glob('GCN_checkpoint_separateplayers_Jul7_*.pt')
    if checkpoint_files:
    ...
    for episode in range(start_episode, num_episodes):
        ...
        ppo_update([white_trajectories, black_trajectories], model, optimizer)
