...
def save_checkpoint(model, optimizer, epoch, epsilon):
    ...
    while os.path.exists(f"{base_path}_v{version}.pth"):
        version += 1
    path = f"{base_path}_v{version}_canbesaved.pth"
    torch.save({
        'epoch': epoch,
...
if __name__ == "__main__":
    model_path = "policy_net_1050_v0_canbesaved.pth"  # last model trained with non-sparse (turn-steady) rewards
