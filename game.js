...
class Game {
    ...
    endGame(winner) {
        ...
        let score;
        if (winner === 'white') {
            score = TOTAL_PIECES - this.blackSavedRack.pieces.length;
        } else {
            score = TOTAL_PIECES - this.whiteSavedRack.pieces.length;
        }

        console.log(`${winner} wins with a score of ${score}!`);

        // Start the EndGameScene with the winner and score information
...
class EndGameScene extends Phaser.Scene {
    ...
    create() {
        this.add.text(CENTER_X, CENTER_Y - 50, `${this.winner} wins with a score of ${this.score}!`, {
            fontSize: '48px',
            color: '#ff0000'
        }).setOrigin(0.5);
        ...
        restartButton.on('pointerdown', () => {
            this.scene.start('MainGameScene'); // Replace with your main game scene key
        });
        ...
        quitButton.on('pointerdown', () => {
            this.game.destroy(true); // Destroy the game instance
        });
    }
}
