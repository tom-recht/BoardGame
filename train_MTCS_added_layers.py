...
def save_checkpoint(model, optimizer, epoch, epsilon):
    ...
    while os.path.exists(f"{base_path}_v{version}_added_layers.pth"):
        version += 1
...
def train(model_path=None, epsilon_override=None):
    ...
    policy_net = GameNet(input_size, hidden_size1, hidden_size2, output_size)
    target_net = GameNet(input_size, hidden_size1, hidden_size2, output_size)
    ...
    for episode in range(start_epoch, num_episodes):
        ...
        for t in range(max_steps):  # Limit the length of each episode
            ...
            action = 1   # hack to stop it picking action 1 which translates to an impossible move, don't know why it's doing this
            while action == 1:
